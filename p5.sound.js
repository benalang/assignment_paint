...
(function(root, factory) {
  ...
  Tone_core_Tone = function() {
    ...
    Tone.prototype.defaultArg = function(given, fallback) {
      if (typeof given === 'object' && typeof fallback === 'object') {
        ...
        for (var givenProp in given) {
          ret[givenProp] = this.defaultArg(given[givenProp], given[
            givenProp]);
        }
    ...
    Tone._initAudioContext(function(audioContext) {
      Tone.prototype.bufferTime = Tone.prototype.bufferSize /
  ...
  Tone_signal_Signal = function(Tone) {
    ...
    Tone.Signal.prototype.linearRampToValueAtTime = function(value,
      endTime) {
      value *= this._syncRatio;
      this._scalar.gain.linearRampToValueAtTime(value, this.toSeconds(
        endTime));
    };
    Tone.Signal.prototype.exponentialRampToValueAtTime = function(value,
      ...
      value *= this._syncRatio;
      try {
        this._scalar.gain.exponentialRampToValueAtTime(value, this.toSeconds(
          endTime));
      } catch (e) {
        this._scalar.gain.linearRampToValueAtTime(value, this.toSeconds(
          endTime));
    ...
    Tone.Signal.prototype.exponentialRampToValueNow = function(value,
      endTime) {
      var now = this.now();
    ...
    Tone.Signal.prototype.linearRampToValueNow = function(value, endTime) {
      ...
      this._scalar.gain.linearRampToValueAtTime(value, now + this.toSeconds(
        endTime));
    };
    Tone.Signal.prototype.setTargetAtTime = function(value, startTime,
      timeConstant) {
      value *= this._syncRatio;
      this._scalar.gain.setTargetAtTime(value, this.toSeconds(startTime),
        timeConstant);
    };
    Tone.Signal.prototype.setValueCurveAtTime = function(values,
      startTime, duration) {
      for (var i = 0; i < values.length; i++) {
        values[i] *= this._syncRatio;
      }
      this._scalar.gain.setValueCurveAtTime(values, this.toSeconds(
        startTime), this.toSeconds(duration));
    };
  ...
  oscillator = function() {
    ...
    p5.Oscillator.prototype.phase = function(p) {
      ...
      var now = p5sound.audiocontext.currentTime;
      this.dNode.delayTime.linearRampToValueAtTime(p5.prototype.map(p,
        0, 1, 0, 1 / this.oscillator.frequency.value), now);
    };
    // ========================== //
  ...
  env = function() {
    ...
    p5.Env.prototype.play = function(unit, secondsFromNow) {
      ...
      this.control.linearRampToValueAtTime(this.dLevel, t + this.aTime +
        this.dTime);
      // hold sustain level
      this.control.linearRampToValueAtTime(this.sLevel, t + this.aTime +
        this.dTime + this.sTime);
      // release
      this.control.linearRampToValueAtTime(this.rLevel, t + this.aTime +
        this.dTime + this.sTime + this.rTime);
    ...
    p5.Env.prototype.triggerRelease = function(unit, secondsFromNow) {
      ...
      if (now - this.lastAttack < this.aTime) {
        ...
        this.control.linearRampToValueAtTime(this.sLevel, t + a + this.dTime +
          this.sTime);
        this.control.linearRampToValueAtTime(this.rLevel, t + a + this.dTime +
          this.sTime + this.rTime);
        ...
        this.control.linearRampToValueAtTime(this.rLevel, t + d + 0.01 +
          this.rTime);
        relTime = t + this.sTime + this.rTime;
      } else if (now - this.lastAttack < this.aTime + this.dTime + this
        .sTime) {
      ...
      var clearTime = t + this.aTime + this.dTime + this.sTime + this.rTime;
      // * 1000;
      if (this.connection && this.connection.hasOwnProperty(
          'oscillator')) {
        sourceToClear = this.connection.oscillator;
        sourceToClear.stop(clearTime + 0.01);
      } else if (this.connect && this.connection.hasOwnProperty(
          'source')) {
  ...
  audioin = function() {
    ...
    p5.AudioIn.prototype.start = function() {
      ...
      if (p5sound.inputSources[self.currentSource]) {
        ...
        navigator.getUserMedia(constraints, this._onStream = function(
          stream) {
          self.stream = stream;
          self.enabled = true;
  ...
  Tone_core_Clock = function(Tone) {
    ...
    Tone.Clock = function(rate, callback) {
      ...
      this._jsNode = this.context.createScriptProcessor(this.bufferSize,
        1, 1);
    ...
    Tone.Clock.prototype._processBuffer = function(event) {
      ...
      for (var i = 0; i < bufferSize; i++) {
        ...
        if (sample > 0 && !upTick) {
          upTick = true;
          setTimeout(function() {
            var tickTime = now + self.samplesToSeconds(i +
              bufferSize * 2);
            return function() {
              self.tick(tickTime);
            };
  ...
  src_app = function() {
    ...
    var p5SOUND = sndcore;
    return p5SOUND;
  }(sndcore, master, helpers, panner, soundfile, amplitude, fft, signal,
    oscillator, env, pulse, noise, audioin, filter, delay, reverb, metro,
    looper, soundRecorder, peakdetect, gain);
}));
